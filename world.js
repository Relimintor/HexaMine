...
const dot = x * 127.1 + y * 311.7 + z * 74.9 + h * 0.00001;
...
function getBiomeRGB(elevation, isPentagon) {
if (isPentagon) return [230, 176, 75];
if (elevation < 0.30) return [109, 199, 109];
if (elevation < 0.50) return [140, 106, 69];
if (elevation < 0.70) return [143, 147, 153];
return [232, 232, 232];
}

function getSideRGB(elevation, isPentagon) {
if (isPentagon) return [184, 137, 58];
if (elevation < 0.30) return [106, 74, 45];
if (elevation < 0.50) return [95, 68, 42];
if (elevation < 0.70) return [99, 102, 107];
return [176, 176, 176];
}
...
polygons
  ...
  .forEach((polygon) => {
    ...
    const sideRGB = getSideRGB(polygon.tile.elevation, polygon.tile.isPentagon);
    ...
    for (let i = 0; i < polygon.topCorners.length; i += 1) {
      ...
      ctx.closePath();
      ctx.fillStyle = shadeColor(sideRGB[0], sideRGB[1], sideRGB[2], sideShade);
      ctx.fill();
    }

    const topShade = Math.max(0.45, Math.min(1, polygon.zAverage / 2.6));
    const topRGB = getBiomeRGB(polygon.tile.elevation, polygon.tile.isPentagon);
    ctx.beginPath();
    polygon.projected.forEach((point, index) => {
      if (index === 0) {
    ...
    ctx.fillStyle = shadeColor(topRGB[0], topRGB[1], topRGB[2], topShade);
    ctx.fill();
